...
# Form implementation generated from reading ui file 'LoginPage_teacher.ui'
#
...
from Final_UI.Teacher import Ui_Teacher_Signup
...
class Ui_LoginPageTeacher(object):
    ...
    def jump_signuppage(self):
        self.Teacher_Signup = QtWidgets.QDialog()
        self.ui = Ui_Teacher_Signup()
        self.ui.setupUi(self.Teacher_Signup)
        self.Teacher_Signup.show()
    ...
    def login_clicked(self,role):
        ...
        if (us.isalnum() and pa.isalnum()):
            query = "select name from teachers where erp = %s and pass = %s"
            inputs = (us,pa)
        ...
        if not self.result:
            ...
            query = "update teachers set status = 'A' where name = %s"
            mycursor.execute(query,(self.result))
            self.mydb.commit()

            ip = socket.gethostbyname(socket.gethostname())
            query = "update teachers set ip = %s where name = %s"
...
"")
    ...
    def __del__(self):
        ...
        print("Destructed")
        if self.result != "":
            query = "update teachers set status = 'NA',ip = NULL where name = %s"
            inputs = self.result
            mycursor.execute(query,inputs)
            self.mydb.commit()
            self.mydb.close()
